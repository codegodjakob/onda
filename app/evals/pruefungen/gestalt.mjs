...
//
// Am 06.08.2026 auf die neu gebaute Oberfläche nachgezogen. Die Zusagen sind Wort für Wort
// dieselben geblieben; geändert hat sich nur, WORAN gemessen wird. Die Lehre aus diesem
// Nachziehen steht in jedem Abschnitt noch einmal einzeln, weil sie teuer war:
//
...
try {
  ...
  // Anker ist die Trägerebene #localAgentLayer: das ist der Ort, an den die App ihre
  ...
  // und meldete grün. Genau dieser Fall soll hier nie wieder moeglich sein, deshalb
  ...
  const doppelung = await page.evaluate(() => {
    ...
    const bausteine = [...spalte.querySelectorAll('[aria-expanded]')]
    ...
    for (const knoten of spalte.querySelectorAll('*')) {
      ...
      if (!quelle) continue
      const baustein = knoten.closest('[aria-expanded]')
  ...
  //
  // Anker ist wieder #structureNav, und zwar ueber die Aufklapp-Semantik
  // (button[aria-expanded]) statt ueber eine Klasse. Der fruehere Ausdruck
  ...
  // "die uebrigen bleiben unveraendert" faelschlich gerissen haette.
  const aufklappen = await page.evaluate(async () => {
    const warte = () => new Promise(r => setTimeout(r, 350))
    const spalte = document.getElementById('structureNav')
    if (!spalte) return { fehler: 'Strukturbereich #structureNav fehlt' }
    const karten = [...spalte.querySelectorAll('button[aria-expanded]')]
  ...
  const versalien = await page.evaluate(() => {
    ...
    //
    // Die Ausnahme haengt am Schriftgrad, nicht nur am Klassennamen: sonst genuegte
    // es, eine grosse Versalien-Ueberschrift "eyebrow" zu nennen, um an der Pruefung
    // vorbeizukommen. Erlaubt ist die Ausnahme nur dort, wo sie das Design System
    // auch gibt — bis 12px.
    const RUBRIK_MAX_PX = 12
    const istRubrik = e => e.matches('[class*=eyebrow], [class*=kicker]')
      && parseFloat(getComputedStyle(e).fontSize) <= RUBRIK_MAX_PX
  ...
  // Selektors (/onda-home|library|home/i). Das ist dasselbe Raten wie bei den
  // Klassennamen oben: von 32 Regeln, die die Startseite wirklich gestalten, erfasste
  // der Wortlaut nur 26 — und ausgerechnet die sechs uebersehenen trugen jeden
  ...
  const scanne = () => page.evaluate(() => {
    ...
    return { markenGenutzt: [...marken], geprueft, festeWerte: roh, eintraege: bibliothek.querySelectorAll('#doclist .doc, #trashlist .trash-doc').length }
  })
