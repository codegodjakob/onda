...
// Regel-2-Ausnahmen — beim Merge von Issue #12 nach main am 8.8.2026 entstanden.
//
// live-native-probe.mjs schickt EINE Anfrage direkt ueber den Transport, am Gateway und
// damit am Tor vorbei. Sie entstand, waehrend der Tor-Zweig auf der Halde lag, und war
// deshalb nie gegen diesen Waechter geprueft. Es ist ein ECHTES Loch: die Sonde kostet
// Geld und steht in keinem Journal.
//
// Warum sie trotzdem drinsteht statt sofort umgebaut zu werden: die Sonde ist bewusst
// schmal gebaut — einmalig, ohne Wiederholung, ohne Persistenz, und sie gibt weder Eingabe
// noch Ausgabe noch Fehlertexte zurueck, nur nicht-sensible Metadaten. Sie durchs Tor zu
// fuehren heisst, diese Zusagen neu zu pruefen; das ist eine eigene Aufgabe und kein
// Nebenprodukt eines Merges.
//
// DIESE LISTE DARF NUR SCHRUMPFEN. Ein neuer Eintrag waere ein neuer, stillschweigend
// genehmigter Weg am Tor vorbei — genau die Fehlerklasse, gegen die dieser Waechter steht.
const TRANSPORT_AUSNAHMEN = ['live-native-probe.mjs']
...
function pruefeQuelle(dateiname, quelle) {
  ...
  if (dateiname !== 'agent-gateway.mjs' && !TRANSPORT_AUSNAHMEN.includes(dateiname)) {
...
// Dieselbe Sperrklinke wie bei den runTask-Ausnahmen: die Liste ist festgenagelt, damit
// niemand eine weitere Datei am Gateway vorbeifuehrt, ohne dass dieser Test rot wird.
test('die Transport-Ausnahmen duerfen nur schrumpfen', () => {
  assert.deepEqual(
    TRANSPORT_AUSNAHMEN,
    ['live-native-probe.mjs'],
    'Ein neuer Eintrag ist ein neuer bezahlter Weg am Tor und am Journal vorbei. '
    + 'Faellt die Sonde weg oder laeuft sie durchs Tor, muss diese Liste LEER werden.',
  )
})
