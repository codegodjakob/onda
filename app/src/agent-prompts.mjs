...
- Jeder Hinweis füllt alle Felder: kategorie, anmerkungsart, anker (wörtliches Minimal-Zitat), beobachtung (was dir auffällt), relevanz (warum es für Ziel und Publikum zählt), folge (was passiert, wenn es bleibt), muster, vorschlagsart, stilmittelId, vorschlag, istGrundursache, integritaet, gewinn, verschiebung.
- verschiebung nennt das ZIEL eines Ortswechsels und wird nur bei anmerkungsart "verschieben" gefüllt, sonst ist sie null. zielAnker ist ein wörtliches Minimal-Zitat aus dem Absatz, NEBEN den verschoben werden soll — nicht aus dem Absatz, der wandert; lage sagt "davor" oder "danach". Findest du keine Zielstelle, die du wörtlich zitieren kannst, gib den Hinweis nicht als "verschieben" — ein Ortswechsel ohne Ziel ist keiner und wird verworfen.
