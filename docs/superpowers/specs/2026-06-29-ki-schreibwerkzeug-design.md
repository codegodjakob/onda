...
**0a2. Einheitlichkeit & wenigste Klicks (Gestaltungsregel, vom Nutzer festgelegt).** Gleiche/ähnliche Elemente verhalten sich überall gleich (z. B. Listenzeilen: immer dieselben Hover-Aktionen am selben Ort). Die Interaktion mit den wenigsten Klicks gewinnt; auf einen Blick erkennbar schlägt versteckt im Menü. Keine Funktions-Doppelungen an mehreren Orten.

**0a3. Seiten-Logik (vom Nutzer festgelegt).** Die **Bibliothek** (Startseite) verwaltet Projekte/Texte/Papierkorb — die **Schreibansicht** dient nur dem Schreiben; ihre linke Spalte gehört später der Meta-Struktur (Spine), nicht dem Textverzeichnis. Leisten-Logik: **oben alles, was das Schriftbild/den Text formt** (Absatzformat, Zeichenformat, Schrift, Einfügen) · **Zahnrad nur App-Verhalten** (Erscheinung, Zeilenbreite, Rechtschreibung, Fokus, Export/Drucken).
