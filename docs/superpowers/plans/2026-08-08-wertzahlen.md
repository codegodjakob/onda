...
- `bilanziereRueckmeldung({ dokumente })` liefert `{ proArt: Map(art → { angeboten, angenommen, verworfen, risikoAngenommen, bewertbar, verworfenAnteil, lage, … }), gesamt, handwerk }` (rueckkopplung-model.mjs:185ff); Schwellen dort: MINDESTZAHL_JE_ART=5 usw. — für die ERTRAG-Anzeige gilt die eigene 10er-Regel aus dem Issue.
