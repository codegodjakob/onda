...
**Goal:** Build and verify the complete monochrome Onda design documentation inside the existing Figma file `Claude Code` (`0DbO0vK6shrVU2qkmWSxIp`), exclusively on its existing `Page 1`, including all 29 annotation types, multi-state dialogs, responsive references, and clickable workflows.
...
- Target file key: exactly `0DbO0vK6shrVU2qkmWSxIp`.
- Target URL: `https://www.figma.com/design/0DbO0vK6shrVU2qkmWSxIp/Claude-Code?node-id=0-1`.
- Target page: exactly the existing `Page 1`.
- Preserve the complete baseline page inventory: page count, IDs, names, and order. Do not create a new Figma page of any name, and do not rename, reorder, or delete a page.
- Preserve every pre-existing node across the entire file. Build a canonical recursive snapshot rooted at the existing `DOCUMENT`/`PAGE` nodes and a SHA-256 hash for every baseline node. For every node capture ordered baseline child IDs, ID, name, type, parent ID, absolute and local bounds, `visible`, `opacity`, text, fills, strokes, effects, component/instance references, and relevant Auto Layout properties (`layoutMode`, axis sizing/alignment, `itemSpacing`, all paddings, `layoutPositioning`, `layoutAlign`, `layoutGrow`, and constraints where present).
- On comparison, create a baseline projection of the fresh full-file snapshot that filters only newly added direct Onda siblings from `Page 1` child lists; the projected child order, every captured property, and every per-node hash for baseline nodes must remain identical.
- Before mutation, compute the occupied bounds from the baseline snapshot. Place the Onda content area beyond those bounds with at least 2,000 px clearance.
- Every required Onda content section uses exactly the canonical name listed below without a prefix, has type `SECTION` or the explicitly recorded fallback type `FRAME`, and is a direct child of `Page 1`; do not nest required sections inside another node.
- All visible paints are neutral grayscale values; solid RGB colors satisfy `r === g && g === b` within `0.002`.
- Base surfaces and lists use 0 px radius; controls use 4 px; larger static surfaces use exactly 6 px; overlays use 8 px; 999/full radius is limited to true circles.
...
|---|---|---|
| AC-1 · One-page target | File key is `0DbO0vK6shrVU2qkmWSxIp`; page count, IDs, names, and order exactly match the baseline; no page of any name was added; every canonical Onda section is a direct child of `Page 1` with type `SECTION` or recorded fallback `FRAME` | complete before/after page inventory plus section name/type/parent inventory |
| AC-2 · Existing content preserved | A canonical recursive full-file snapshot and per-node SHA-256 hashes prove every prior `DOCUMENT`, `PAGE`, and Scene node unchanged, including baseline child order and all specified visual, content, reference, and Auto Layout properties | per-node hashes and property-level diff of the baseline projection |
...
- [ ] Confirm the active file name is `Claude Code`, file key is `0DbO0vK6shrVU2qkmWSxIp`, URL is `https://www.figma.com/design/0DbO0vK6shrVU2qkmWSxIp/Claude-Code?node-id=0-1`, and selected page name is `Page 1`.
- [ ] Record the complete ordered page inventory before mutation: count plus every page's index, ID, and name. Expected after mutation: byte-for-byte equivalent canonical inventory; no new page of any name.
- [ ] Serialize the complete existing file recursively from its `DOCUMENT`/`PAGE` nodes. Per node include ordered baseline child IDs, ID/name/type/parent, local and absolute bounds, `visible`, `opacity`, text, fills/strokes/effects, component/instance references, and relevant Auto Layout properties. Canonicalize object keys without sorting child arrays and store a SHA-256 hash for every baseline node.
- [ ] Compute the union of occupied bounds on `Page 1` from the recursive baseline and choose a placement origin at least 2,000 px beyond it.
...
- [ ] Establish failing contract tests for the exact file key, immutable full page inventory, recursive preservation hashes, canonical unprefixed section names/types/parents, grayscale paints, allowed radii, dialog rows, responsive widths, annotation states, and idempotent reruns.
- [ ] Implement a generator that aborts unless file key is exactly `0DbO0vK6shrVU2qkmWSxIp` and the active page is exactly `Page 1`; it must never call `figma.createPage()`.
...
- [ ] Make the generator report created/updated node IDs, final Onda bounds, complete page inventory, canonical recursive baseline/post snapshots, per-node SHA-256 hashes, and validation failures.
...
- [ ] Create canonically named `00 · Übersicht`, `01 · Foundations`, and `02 · Komponenten` with type `SECTION` or recorded `FRAME` fallback as direct children of `Page 1` at the calculated origin.
...
- [ ] Create each canonical, unprefixed `05.01`–`05.24` and `06.01`–`06.05` required section exactly once, with type `SECTION` or recorded `FRAME` fallback and direct parent `Page 1`.
...
- [ ] Re-read the complete ordered page inventory. Hard gate: target key remains `0DbO0vK6shrVU2qkmWSxIp`; page count, IDs, names, and order exactly equal the baseline; no page of any name was added.
- [ ] Rebuild the canonical recursive full-file snapshot and every per-node SHA-256 hash. Project it onto the Baseline by filtering only new direct Onda siblings from the `Page 1` child comparison. Hard gate: every baseline node hash and captured property is unchanged, including baseline child order.
